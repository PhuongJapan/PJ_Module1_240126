  ...
  for (let index = firstIndex; index < lastIndex; index++) {
    listClass.innerHTML += `
            <tr>
                ...
                <td>
                    <button id="btnEditClass" class="btn btn-success" onclick="displayDataToEdit('${arrClass[index].classId
      }')"> 
                    <i class="fa-solid fa-pen-to-square" ></i></button>
                    <button id="btnDeleteClass" class="btn btn-danger" onclick="deleteClass('${arrClass[index].classId}')"><i class="fa-solid fa-trash"></i></button>
                </td>
            </tr>
        `;
  }
...
function createClass() {
  ...
  if (!validateClassId(newClass.classId)) {
    return;
  }
...
function editClassInfo() {
  ...
  let arrCourse = localStorage.getItem("studentManagement")
    ? JSON.parse(localStorage.getItem("studentManagement"))
    : [];
  let courseIndex = getCourseIdOfClass(arrCourse, idEdit);
  console.log(courseIndex)
  let editClassInfo = getDataFormEdit();
  let index = getClassById(arrCourse, idEdit);
  console.log(index);
  if (index > -1) {
    arrCourse[courseIndex].arrClass[index] = editClassInfo;
  }
...
function getDataFormEdit() {
  ...
  arrCourse[courseIndex].arrClass[classIndexUpdate].className = className;
  arrCourse[courseIndex].arrClass[classIndexUpdate].descriptions = descriptions;
  arrCourse[courseIndex].arrClass[classIndexUpdate].studentNumber = studentNumber;
  arrCourse[courseIndex].arrClass[classIndexUpdate].teacher = teacher;
  return arrCourse[courseIndex].arrClass[classIndexUpdate];
}
